/-
  ...
  of FASS.DataSpectre; every `theorem` after it is the corresponding all-levels statement, obtained
...
-- Not used below: imported so that lake builds this file after FASS.Hex rather than beside it
-- (each of the two needs about 9 GB at its peak; see lean/README.md).
import FASS.Hex
...
theorem base_lab : closure.all (holdsLB 2 L1) = true := by decide +kernel
