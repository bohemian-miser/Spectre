...
const sketch = (p) => {
    ...
    function buildSpectreBase(curved) {
        ...
        if (curved) {
            mystic.addChild(new CurvyShape(spectre, spectre_keys, 'Gamma1'), ident);
            mystic.addChild(new CurvyShape(spectre, spectre_keys, 'Gamma2'), mul(ttrans(spectre[8].x, spectre[8].y), trot(p.PI / 6)));
        }
        else {
            mystic.addChild(new Shape(spectre, spectre_keys, 'Gamma1'), ident);
            mystic.addChild(new Shape(spectre, spectre_keys, 'Gamma2'), mul(ttrans(spectre[8].x, spectre[8].y), trot(p.PI / 6)));
        }
    ...
    function buildHatTurtleBase(hat_dominant) {
        ...
        if (hat_dominant) {
            ...
            mystic.addChild(new Shape(hat, hat_keys, 'Gamma2'), mul(ttrans(turtle[9].x, turtle[9].y), trot(p.PI / 3)));
    ...
    function buildSupertiles(sys) {
        ...
        for (const [ang, from, to] of t_rules) {
            ...
            const ttt = ttrans(transPt(Ts[Ts.length - 1], quad[from]).x - tquad[to].x, transPt(Ts[Ts.length - 1], quad[from]).y - tquad[to].y);
            Ts.push(mul(ttt, rot));
        }
    ...
    p.setup = () => {
        ...
        try {
            colscheme_sel.elt.dispatchEvent(new Event('change'));
        }
        catch (e) { }
        ...
        function drawGeomToContext(ctx, geom, S, overrideLabel) {
            S = mul(S, adjust_mat);
            if (!geom)
                return; // guard against missing sys[label]
            if (geom.geoms && Array.isArray(geom.geoms)) {
                for (let g of geom.geoms) {
                    if (g && g.geom)
                        drawGeomToContext(ctx, g.geom, mul(S, g.xform), overrideLabel);
                }
                return;
            }
            // shape-like
            ...
            if (window.showBackgrounds) {
                ctx.fillStyle = `rgb(${col[0]},${col[1]},${col[2]})`;
                ctx.fill();
            }
            if (window.showOutlines) {
            ...
            if (typeof overlays !== 'undefined' && overlays[overlayKey] && window.showLines) {
                ctx.strokeStyle = 'black';
                ctx.lineWidth = 2;
            ...
            try {
                if (window.selectedMajorEdges && window.selectedMajorEdges.size > 0 && unique_edge_labels) {
        ...
        function makeThumbnail(label) {
            ...
            el.elt.style.width = thumbWidth + 'px';
            el.elt.style.height = thumbHeight + 'px';
            ...
            el.elt.addEventListener('pointermove', function (ev) {
                if (!activeStroke.points || activeStroke.canvas !== el.elt)
                    return;
                ev.preventDefault();
                const rect = el.elt.getBoundingClientRect();
                const x = ev.clientX - rect.left;
        ...
        if (miniNames.length > 10)
            miniNames = miniNames.slice(0, 10);
        ...
        window.selectedMajorEdges = new Set();
        ...
        showOutlinesCheck.changed(() => {
            window.showOutlines = showOutlinesCheck.checked();
            refreshThumbnails();
            p.loop();
        });
        ...
        showBackgroundsCheck.changed(() => {
            window.showBackgrounds = showBackgroundsCheck.checked();
            refreshThumbnails();
            p.loop();
        });
        ...
        showLinesCheck.changed(() => {
            window.showLines = showLinesCheck.checked();
            refreshThumbnails();
            p.loop();
        });
        ...
        let y_pos = 260;
        const majorEdgeCheckboxes = [];
        for (const edge_type of edge_types) {
            const checkbox = p.createCheckbox(edge_type.label, false);
            checkbox.position(10, y_pos);
            checkbox.changed(() => {
                if (checkbox.checked()) {
                    window.selectedMajorEdges.add(edge_type.value);
                }
                else {
                    window.selectedMajorEdges.delete(edge_type.value);
                }
                refreshThumbnails();
                p.loop();
            });
        ...
        lbl_check.changed(() => {
            const isChecked = lbl_check.checked();
            majorEdgeCheckboxes.forEach((checkbox, index) => {
                checkbox.checked(isChecked);
                if (isChecked) {
                    window.selectedMajorEdges.add(edge_types[index].value);
                }
                else {
                    window.selectedMajorEdges.clear();
                }
            });
            refreshThumbnails();
            p.loop();
    ...
    p.draw = () => {
        ...
        p.applyMatrix(to_screen[0], to_screen[3], to_screen[1], to_screen[4], to_screen[2], to_screen[5]);
        const s = colscheme_sel.value();
        if (s == 'Figure 5.3') {
            colmap = colmap53;
        }
