...
/*
	...
	*/
	...
	// Create color pickers for each tile type
	const sidebar = select('#sidebar');
	for (const tileName of tile_names) {
		const container = createDiv();
		container.parent(sidebar);

		const label = createSpan(`${tileName}: `);
		label.parent(container);

		const colorPicker = createInput(colmap_orig[tileName], 'color');
		colorPicker.parent(container);
		colorPicker.input(() => {
			custom_colors[tileName] = colorPicker.value();
...
function draw()
{
	...
	if( s == 'Figure 5.3' ) {
		...
		for (const tileName in custom_colors) {
			const hex = custom_colors[tileName];
